...
from agri_sql_agent.db import create_app_pool, create_agri_pool
from agri_sql_agent.graph import compile_agent_with_postgres
from agri_sql_agent.llm import get_llm
from agri_sql_agent.db_executor import DBExecutor
from agri_sql_agent.logger import get_logger
from agri_sql_agent.exceptions import ResourceInitializationError
...
async def startup() -> AppContext:
    ...
    try:
        ...
        logger.info("Creating Agri DB Pool (asyncpg)...")
        _ctx.agri_pool = await create_agri_pool()

        # 3. Wrap the agri pool in the DBExecutor (SOLID dependency injection target)
        _ctx.db_executor = DBExecutor(_ctx.agri_pool)
...
async def shutdown() -> None:
    ...
    try:
        if _ctx.agri_pool:
            await _ctx.agri_pool.close()
            logger.info("Closed Agri DB Pool.")
        if _ctx.app_pool:
