...
from talk2db.schemas import RoutingOutput, SQLOutput
from talk2db.guardrails import validate_and_format_sql
from talk2db.exceptions import SQLSecurityViolation, DatabaseExecutionError, LLMGenerationError, ResourceInitializationError
...
FEW_SHOT_ROUTER = load_prompt("few_shot_router.txt")
FEW_SHOT_SQL_GEN = load_prompt("few_shot_sql_gen.txt")
...
async def router_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    prompt = ROUTER_PROMPT.format(history=history, user_query=last_user_message, schema=SCHEMA_PROMPT, few_shot_examples=FEW_SHOT_ROUTER)
...
async def sql_gen_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    prompt = SQL_GEN_PROMPT.format(
        ...
        few_shot_examples=FEW_SHOT_SQL_GEN,
...
async def refusal_node(state: AgentState) -> Dict[str, Any]:
    ...
    """
    msg = AIMessage(
        content="I'm sorry, but I can only answer questions related to the provided "
                "database schema."
    )
